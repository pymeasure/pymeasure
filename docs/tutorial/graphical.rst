...
Using the MultiplotWindow
~~~~~~~~~~~~~~~~~~~~~~~~~

The MultiplotWindow allows you to view multiple plots at one time. This can be useful for viewing data from multiple instruments at the same time.

A limitation of MultiplotWindow is that all X axis are shared across plots

Below we can make minor modifications to our previous example to use a MultiplotWindow. ::
    ...
    from pymeasure.log import console_log
    from pymeasure.display.Qt import QtWidgets
    from pymeasure.display.windows.multiplot_window import MultiplotWindow
    from pymeasure.experiment import Procedure, Results
    from pymeasure.experiment import IntegerParameter, FloatParameter, Parameter
