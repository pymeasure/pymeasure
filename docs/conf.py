...
def get_class_name(cls):
    ...
    second_idx = cls_str[first_idx + 1:].index("'")
    return cls_str[first_idx + 1:first_idx + second_idx + 1]


def gen_channel_docs(app, what, name, obj, options, lines):
    """
        ...
        for attr, channel_class in obj.get_channels(obj):
            if isinstance(channel_class, CommonBase.ChannelCreator):
                ...
                channels = ['``' + prefix + str(id) + '``: :class:`~' + get_class_name(cls) + '`'
                            for cls, id in channel_class.pairs]
                lines += ['.. py:attribute:: ' + attr, '', ]
                lines += ['   :channels: ' + ", ".join(channels), '']
