...
class TestTektronixMSO58:
    ...
    EXPECTED_MEAS_VALUES = {'PK2PK': 2.5, 'PWIDTH': 500.0000E-6, 'NWIDTH': 500.0000E-6, 'FREQUENCY': 1.0000E3}
    AFG_FUNCTIONS = ["SINE", "SQUARE", "PULSE", "RAMP", "NOISE", "DC", "SINC", "GAUSSIAN", "LORENTZ",
    ...
    def test_measurement_configure(self, autoscaled_instrument):
        for (slot, meas_type), (meas, value) in zip(self.MEAS_SLOTS.items(), self.EXPECTED_MEAS_VALUES.items()):
    ...
    def test_afg_amplitude(self, instrument):
        instrument.afg_amplitude = 100.0000E-3
        assert instrument.afg_amplitude == 100.0000E-3
