...
def test_set_safe_output():
    """Test setting a safe voltage and current output."""
    with expected_protocol(
        KepcoKLP75,
        [
            (b"OUTPut:STATe 0", None),
            (b"SOURce:VOLTage 20.0", None),
            (b"SOURce:CURRent 3.0", None),
            (b"OUTPut:STATe 1", None),
        ],
    ) as inst:
        inst.set_safe_output(voltage=20.0, current=3.0)
